...
const WhatsAppButton: React.FC = () => {
  ...
  const [teethingBubble, setTeethingBubble] = useState(false);
  ...
  useEffect(() => {
    ...
    const teethingTimer = setTimeout(() => {
      if (!dismissed) setTeethingBubble(true);
    }, 15000);

    return () => {
      clearTimeout(timer);
      clearTimeout(teethingTimer);
    };
  }, [dismissed]);

  return (
    <div className="fixed bottom-8 right-8 z-[110] flex items-center">
      {(isHovered || (showNotification && !dismissed) || (teethingBubble && !dismissed)) && (
        <div className="relative group bg-white px-6 py-4 rounded-[25px] shadow-2xl mr-4 animate-[slideLeft_0.3s_ease-out] border border-[#E8DED3]/30 min-w-[200px]">
          <button 
            onClick={() => {setShowNotification(false); setTeethingBubble(false); setDismissed(true);}}
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-[#2F2F2F]/40">Expert Help</p>
                <p className="text-sm font-bold text-[#2F2F2F]">
                  {teethingBubble ? "Confused between options? We can help 😊" : "Need help choosing?"}
                </p>
             </div>
          </div>
        </div>
